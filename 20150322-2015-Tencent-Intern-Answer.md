...
答：考察**以O(n)的时间复杂度算逆序数**的算法：逐项读入每一个字母并计数，如果读入的是A，那么就与已经读入的CGT都构成逆序对；如果读入的是C，那么就与已经读入的GT都构成逆序对，以此类推。
```C++
switch(pdna[i]){
case 'A':
	c[0]++;		//更新A的计数器
...
**关键词**：聚类查找而非遍历查找，将本机所有样本按照人物分类，同一个人的放一起。每次对比只和该类的第一张照片进行比对。  
B\. 假设机器使用的数量不限，为了尽快缩短单个请求的时间，你将从哪些方面进行优化？  
**关键词**：总体样本分布式、简单预测试（比如男女、年龄段）并根据预测试结果缩小目标样本范围  
